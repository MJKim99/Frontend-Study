...
function placesSearchCB(data, status, pagination) { // 장소 검색이 완료됐을 때 호출되는 콜백함수
  if (status === kakao.maps.services.Status.OK) { // 정상적으로 검색 완료
    displayPlaces(data); // 검색 목록과 마커를 표출
...
function displayPlaces(places) { // 검색 결과 목록과 마커를 표출
  ...
  for (var i = 0; i < places.length; i++) { // 마커 생성, 지도에 표시
